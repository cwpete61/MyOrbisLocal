...
export async function runApiSuite() {
  ...
  // Use @example.test domain so the admin /api/admin/test-tenants cleanup
  // endpoint can wipe these later. Username is required by the signup schema.
  const signupTimestamp = Date.now()
  const signupEmail    = `e2e-api-${signupTimestamp}@example.test`
  ...
  // here don't affect the test verdict — the @example.test domain is
  ...
  if (createdTenantId) {
    ...
    // The orphan @example.test rows can be cleaned via the admin

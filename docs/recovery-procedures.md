...
```bash
# Dump the template database to a timestamped file
docker exec umoja-postgres pg_dump -U template -d template -F c \
  > backups/db_$(date +%Y%m%d_%H%M%S).dump

# Restore from a dump
docker exec -i umoja-postgres pg_restore \
  -U template -d template --clean --if-exists \
...
docker exec template-postgres pg_dump \
  -U template -d template -F c \
...
5. Restart services and run the E2E suite: `pnpm --filter @template/e2e test`.
...
ssh root@147.93.183.4 'docker exec app_name-postgres pg_dump -U template -d template -F c' > "$DUMP"

# 2. Spin up a temporary PG 16 container on a non-conflicting port
docker run -d --name pg16-restore-test \
  -e POSTGRES_USER=template \
  -e POSTGRES_PASSWORD=test \
  -e POSTGRES_DB=template_restore_test \
  -p 5433:5432 \
  postgres:16-alpine

# 3. Wait for it to be ready
for i in {1..15}; do
  docker exec pg16-restore-test pg_isready -U template 2>&1 | grep -q "accepting" && break
  sleep 1
done

# 4. Restore
docker cp "$DUMP" pg16-restore-test:/tmp/restore.dump
docker exec pg16-restore-test pg_restore -U template -d template_restore_test --no-owner --no-acl /tmp/restore.dump
...
for t in Tenant TenantMember User Plan RoleDefinition Conversation Contact Appointment SystemConfig BusinessProfile; do
  PROD=$(ssh root@147.93.183.4 "docker exec app_name-postgres psql -U template -d template -t -c 'SELECT COUNT(*) FROM \"$t\"'" | tr -d ' \r\n')
  RESTORED=$(docker exec pg16-restore-test psql -U template -d template_restore_test -t -c "SELECT COUNT(*) FROM \"$t\"" | tr -d ' \r\n')
...
3. Verify the build passes: `pnpm --filter @template/api build` and `pnpm --filter @template/web build`.
...
ssh root@147.93.183.4 'docker logs app_name-api --tail=200' > /tmp/broken_api_$(date +%s).log
ssh root@147.93.183.4 'docker logs app_name-web --tail=200' > /tmp/broken_web_$(date +%s).log
```
...
# Compose file
ssh root@147.93.183.4 'cp /opt/app_name/infrastructure/docker/docker-compose.prod.yml{,.broken.bak}'
scp "$LATEST_DR/compose.prod.docker.yml" \
    root@147.93.183.4:/opt/app_name/infrastructure/docker/docker-compose.prod.yml

# Env file
ssh root@147.93.183.4 'cp /opt/app_name/infrastructure/docker/.env.prod{,.broken.bak}'
scp "$LATEST_DR/env.prod.docker" \
    root@147.93.183.4:/opt/app_name/infrastructure/docker/.env.prod
ssh root@147.93.183.4 'chmod 600 /opt/app_name/infrastructure/docker/.env.prod'
```

**Step 2 — Bring services back up:**
```bash
ssh root@147.93.183.4 'cd /opt/app_name/infrastructure/docker && \
...
```bash
ssh root@147.93.183.4 'docker stop app_name-api app_name-gateway'
```

**Step 2 — Pick a dump (most recent good one):**
```bash
ssh root@147.93.183.4 'docker exec app_name-db-backup ls -lt /backups/ | head -10'
...
ssh root@147.93.183.4 "docker exec app_name-db-backup cat /backups/$DUMP" | \
  ssh root@147.93.183.4 'docker exec -i app_name-postgres pg_restore \
    -U template -d template --clean --if-exists --no-owner --no-acl'
```

**Step 4 — Verify schema state:**
```bash
ssh root@147.93.183.4 'docker exec app_name-postgres psql -U template -d template \
...
| Asset | Path |
|---|---|
| Compose file | `/opt/app_name/infrastructure/docker/docker-compose.prod.yml` |
| Env file | `/opt/app_name/infrastructure/docker/.env.prod` |
| Caddy reverse proxy | `/opt/app_name/infrastructure/caddy/Caddyfile` |
| Daily DB backups | volume `app_name_db_backups` (mounted at `/backups/` inside `app_name-db-backup`) |
